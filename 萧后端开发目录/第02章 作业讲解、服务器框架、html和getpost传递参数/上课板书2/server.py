...
def response_for_path(path):
    ...
    """
    r = {
        '/': route_index,                 # 只存函数不存函数的调用route_index() 否则这里两个函数都要执行 浪费资源 没有必要
...
def main():
    ...
    # 字典定义一般用{ 'host'：'',  'port'：3000} 但是这里用{}时，key必须有''号，用（）不需要''号 而且用（）定义时，每个字段尾部都有','，这样规范
...
message=gua                                      post请求方式发送时信息放在body里发送


GET /?message=GUA HTTP/1.1                       get请求方式发送时信息放在路径里发送 多个信息用&隔开
Host: localhost:3000
Connection: keep-alive
